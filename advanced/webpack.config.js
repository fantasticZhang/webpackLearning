...
/*
 ...
 --env.*传进来的参数可以通过options.*获得.
 */
module.exports = (options = {}) =>{
const confjg = require('./config/'+(process.env.npm_config_config || options.config || "default"));
    return  {
        //配置页面入口文件
        entry: {
           vendor: './src/vendor',
        ...
        plugins:[
            ...
            /*
             使用DefinePlugin插件在打包时往代码中插入需要的环境变量,
             DefinePlugin插件的原理很简单, 如果我们在代码中写:console.log(DEBUG)
             它会做类似这样的处理: 'console.log(DEBUG)'.replace('DEBUG', true)
             最后生成:
             console.log(true)
             */
            new webpack.DefinePlugin({
                ...
                CONFIG: JSON.stringify(confjg.runtimeConfig)
            })
        ],
        resolve:{
            /*简化import路径*/
        ...
        devServer:confjg.devServer?{
            //允许其他主机访问：
            host: '0.0.0.0',
            // 配置监听端口
            port: confjg.devServer.port,
            /*
             ...
             */
            proxy: confjg.devServer.proxy,
            historyApiFallback: {
                index: url.parse(confjg.publicPath).pathname,
                /*
                 禁用相应的规则, 使带后缀的文件当不存在时也能fallback到index.html
                 */
                disableDotRule: true
            }
        }:undefined,
        ...
        /*
         运行开发环境是命令行会报一段warning:

         WARNING in asset size limit: The following asset(s) exceed the recommended size limit (250 kB).
         This can impact web performance.
         ...
         这是说建议每个输出的js文件的大小不要超过250k.
         但开发环境因为包含了sourcemap并且代码未压缩所以一般都会超过这个大小,
